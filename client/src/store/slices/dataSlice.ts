...
export const fetchOverview = createAsyncThunk(
    ...
    async (_, { getState, rejectWithValue }) => {
        ...
        if (isCacheValid(state.data.overviewFetchedAt) && state.data.overview.length > 0) {
        ...
        try {
            ...
            if (res.data.success === 1) {
                return { cached: false, data: res.data.data.overview as OverviewItem[] };
            }
...
export const fetchActivities = createAsyncThunk(
    ...
    async (forceRefresh: boolean = false, { getState, rejectWithValue }) => {
        ...
        if (!forceRefresh && isCacheValid(state.data.activitiesFetchedAt) && state.data.activities.length > 0) {
        ...
        try {
            ...
            if (res.data.success === 1) {
                return { cached: false, data: res.data.data.activities as ActivityItem[] };
            }
...
const dataSlice = createSlice({
    ...
    reducers: {
        ...
        updateActivity: (state, action: PayloadAction<{ id: string; updates: Partial<ActivityItem> }>) => {
            ...
            if (index !== -1) {
                state.activities[index] = { ...state.activities[index], ...action.payload.updates };
            }
        },
        clearData: (state) => {
            state.overview = [];
