...
exports.buildSkillProfile = void 0;
const normalize = (value) => value.toLowerCase().replace(/[^a-z0-9]+/g, "").trim();
const round = (value) => Math.round(value);
const buildSkillProfile = (repositories, voiceScores, aggregateTechnologies) => {
    ...
    const finalScore = averageVoiceScore > 0
        ...
        : averageGithubScore;
    const confidenceScore = round(Math.min(100, repoCount * 4 + Math.min(40, aggregateTechnologies.frameworks.length * 8) + Math.min(20, skillsDetected.length * 3)));
    const primaryStack = [
...
exports.buildSkillProfile = buildSkillProfile;
