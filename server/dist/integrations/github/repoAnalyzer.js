...
exports.analyzeRepository = void 0;
const dependencyTechnologyMap = {
...
const collectFromDependencies = (dependencies, frameworks, libraries, languages, skills, matches) => {
    for (const rawDependency of dependencies) {
        ...
        for (const descriptor of descriptors) {
            if (descriptor.kind === "framework")
                frameworks.add(descriptor.label);
            if (descriptor.kind === "library")
                libraries.add(descriptor.label);
            if (descriptor.kind === "language")
                languages.add(descriptor.label);
            if (descriptor.kind === "skill")
                skills.add(descriptor.label);
        }
    }
};
...
const analyzeRepository = (files, repoLanguages) => {
    const languages = new Set();
    const frameworks = new Set();
    const libraries = new Set();
    const skills = new Set();
    const dependencyMatches = new Set();
